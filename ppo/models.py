...
#                                           use_bias=False, activation=activation, name="conv2d_1")
#             self.conv2 = tf.layers.conv2d(self.conv1, 32, kernel_size=[4, 4], strides=[2, 2],
#                                           use_bias=False, activation=activation, name="conv2d_2")
#             hidden = c_layers.flatten(self.conv2)
            hidden = c_layers.flatten(self.observation_cut)
            
            for j in range(num_layers):
                hidden = tf.layers.dense(hidden, h_size, use_bias=False, activation=activation, name= "v_hidden_{}{}".format(i,j))
    ...
    def create_continuous_state_encoder(self, s_size, h_size, num_streams, activation, num_layers):
        ...
        offset = 1 + 20
        #ccthien
        self.action_filter = tf.cast(self.state_in[:,offset:self.a_size+offset] > 0.2, dtype=tf.float32)
        self.state_in_remain = self.state_in[:,self.a_size+offset:]
    ...
    def create_ppo_optimizer(self, probs, old_probs, value, entropy, beta, epsilon, lr, max_step):
        ...
        decay_epsilon = tf.train.polynomial_decay(epsilon, self.global_step,
                                                  max_step, 1e-2,
                                                  power=1.0, name="decay_epsilon")

        r_theta = tf.identity(probs / (old_probs + 1e-10), name="r_theta")
        p_opt_a = tf.identity(r_theta * self.advantage, name="opt_a")
        p_opt_b = tf.identity(tf.clip_by_value(r_theta, 1 - decay_epsilon, 1 + decay_epsilon) * self.advantage, name="opt_b")
        self.policy_loss = -tf.reduce_mean(tf.minimum(p_opt_a, p_opt_b), name="policy_loss")

        self.value_loss = tf.reduce_mean(tf.squared_difference(self.returns_holder,
                                                               tf.reduce_sum(value, axis=1)), name="value_loss")

        decay_beta = tf.train.polynomial_decay(beta, self.global_step,
                                               max_step, 1e-5,
                                               power=1.0, name = "beta")
        self.loss = tf.identity(self.policy_loss + self.value_loss - decay_beta * tf.reduce_mean(entropy), name="loss")
...
class DiscreteControlModel(PPOModel):
    def __init__(self, lr, brain, h_size, epsilon, beta, max_step, normalize, num_layers):
        ...
        self.policy_original = tf.layers.dense(hidden, a_size, activation=None, use_bias=False,
                                      kernel_initializer=c_layers.variance_scaling_initializer(factor=0.01), name="policy_org")
        
#         self.probs_original = tf.nn.softmax(self.policy_original, name="probs_original")
        
        #self.policy = tf.identity(self.policy_original * self.action_filter + ((self.action_filter - 1) * 100), name="policy")
        self.policy = tf.multiply(self.policy_original, self.action_filter)
        self.policy = tf.add(self.policy, tf.multiply(tf.subtract(self.action_filter,1),100), name="policy")
        ...
        self.value = tf.layers.dense(hidden, 1, activation=None, use_bias=False,
                                     kernel_initializer=c_layers.variance_scaling_initializer(factor=1.0), name="value")
        self.value = tf.identity(self.value, name="value_estimate")
